 ...
 *
 */
public abstract class CrudeControllerTest<DTO extends NonRemovableDto> {
	...
	private CrudeRestService<DTO> clubRestService;
	...
	@Before
	public void setUp() {
		clubRestService = getMockService();
		constructPaths();
		constructViews();
	}
	...
	public void findAllIsValid() throws Exception {
		...
		when(clubRestService.findAll()).thenReturn(dtoList);
	...
	public void findByIdIsValid() throws Exception {
		...
		when(clubRestService.findById(dto.getId())).thenReturn(dto);
		...
		verify(clubRestService).findById(dto.getId());
		verifyNoMoreInteractions(clubRestService);
	}
	...
	public void createIsValid() throws Exception {
		...
		when(clubRestService.create(any(getDtoType()))).thenReturn(dto);

		MockHttpServletRequestBuilder request = post(pathBase)
				.contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.param("enabled", String.valueOf((dto.isEnabled())));
	...
	public void updateIsValid() throws Exception {
		...
		when(clubRestService.update(any(getDtoType()))).thenReturn(dto);
	...
	public void updateClubNotFound() throws Exception {
		...
		when(clubRestService.update(any(getDtoType()))).thenThrow(new EntityNotFoundException("message"));
		...
		verify(clubRestService).update(any(getDtoType()));
		verifyNoMoreInteractions(clubRestService);
	}
	...
	public void disableIsValid() throws Exception {
		...
		when(clubRestService.disable(dto.getId())).thenReturn(dto);

		mockMvc.perform(post(pathDisable, dto.getId())
				.contentType(MediaType.APPLICATION_FORM_URLENCODED))
				.andExpect(status().isFound())
				.andExpect(view().name("redirect:" + pathBase))
				.andExpect(redirectedUrl(pathBase));

		verify(clubRestService).disable(dto.getId());
		verifyNoMoreInteractions(clubRestService);
	}
	...
	public void disableClubNotFound() throws Exception {
		...
		when(clubRestService.disable(dto.getId())).thenThrow(new EntityNotFoundException("message"));
	...
	public void enableIsValid() throws Exception {
		...
		when(clubRestService.enable(dto.getId())).thenReturn(dto);

		mockMvc.perform(post(pathEnable, dto.getId())
				.contentType(MediaType.APPLICATION_FORM_URLENCODED))
				.andExpect(status().isFound())
				.andExpect(view().name("redirect:" + pathBase))
				.andExpect(redirectedUrl(pathBase));

		verify(clubRestService).enable(dto.getId());
		verifyNoMoreInteractions(clubRestService);
	}
	...
	public void enableClubNotFound() throws Exception {
		...
		when(clubRestService.enable(dto.getId())).thenThrow(new EntityNotFoundException("message"));
	 ...
	 *             <code>ResultActions</code>.
	 */
	protected void checkModelList(ResultActions result, DTO target) throws Exception {
		result.andExpect(model().attribute("entityList", hasItem(
				allOf(
						hasProperty("id", is(target.getId())),
						hasProperty("enabled", is(target.isEnabled()))))));
	}
	 ...
	 *             <code>ResultActions</code>.
	 */
	protected void checkModel(ResultActions result, DTO target) throws Exception {
		result.andExpect(model().attribute("entity", hasProperty("id", is(target.getId()))))
